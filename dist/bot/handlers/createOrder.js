...
function buildOrderText(order) {
    const parts = [];
    if (order.tariff)
        parts.push(`Тариф: ${order.tariff}`);
    if (order.date)
        parts.push(`Дата: ${order.date}`);
    if (order.time)
        parts.push(`Время: ${order.time}`);
    if (order.address)
        parts.push(`Адрес: ${order.address}`);
    if (order.people)
...
function buildFullOrderText(order) {
    const parts = [];
    if (order.tariff)
        parts.push(`Тариф: ${order.tariff}`);
    if (order.date)
    ...
    if (order.totalCost)
        parts.push(`Стоимость: ${order.totalCost}`);
    if (order.remainingPayment)
        parts.push(`Остаток оплаты: ${order.remainingPayment}`);
    return parts.join("\n");
}
export function setupCreateOrder(bot) {
    ...
    bot.on("message:text", async (ctx, next) => {
        ...
        if (editOrderId) {
            ...
            const updatedData = {};
            if (newData.date)
                updatedData.date = newData.date;
            if (newData.time)
                updatedData.time = newData.time;
            if (newData.address)
                updatedData.address = newData.address;
            if (newData.price)
                updatedData.price = newData.price;
            if (newData.comment)
                updatedData.comment = newData.comment;
            if (newData.tariff)
                updatedData.tariff = newData.tariff;
            if (newData.clientContact)
                updatedData.clientContact = newData.clientContact;
            if (newData.people)
                updatedData.people = newData.people;
            if (newData.totalCost)
                updatedData.totalCost = newData.totalCost;
            if (newData.remainingPayment)
                updatedData.remainingPayment = newData.remainingPayment;
            ...
            for (const oh of order.hosts) {
                await ctx.api.sendMessage(oh.host.telegramId, `📝 Заказ №${editOrderId} был изменён:\n\n${buildFullOrderText(order)}`);
            }
            return;
        }
        // создание заказа
        ...
        for (const host of hosts) {
            ...
            await ctx.api.sendMessage(host.telegramId, `📢 Новый заказ №${order.id}:\n\n${buildPublicOrderText(order)}`, {
    ...
    bot.callbackQuery(/^accept_(\d+)$/, async (ctx) => {
        ...
        await ctx.editMessageText(`📦 Заказ №${orderId} принят (${newCount}/${order.slots}): ${hostNames}`);
        await ctx.api.sendMessage(host.telegramId, `✅ Заказ №${orderId} принят (${newCount}/${order.slots})\n\n${buildFullOrderText(order)}`);
        if (isFull) {
            await ctx.api.sendMessage(MANAGER_ID, `🎉 Заказ №${orderId} полностью заполнен ведущими!\n\n👤 Ведущие: ${hostNames}\n\n${buildFullOrderText(order)}`);
        }
        else {
            await ctx.api.sendMessage(MANAGER_ID, `📦 Заказ №${orderId} принят (${newCount}/${order.slots})\n\n👤 Ведущий: ${host.name}\n\n${buildFullOrderText(order)}`);
        }
    });
    // отказ
