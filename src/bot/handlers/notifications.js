...
export function startNotifications(bot) {
    setInterval(async () => {
        const now = new Date();
...
export function setupReminders(bot) {
...
async function sendReminders(bot) {
    ...
    for (const [hostId, hostOrdersList] of hostOrders) {
        const host = hostOrdersList[0].hosts.find((h) => h.hostId === hostId)?.host;
        if (!host)
            continue;
        const message = `🔔 Напоминание о завтрашних заказах:\n\n`;
        const ordersText = hostOrdersList.map(order => `📦 Заказ №${order.id}\n${buildFullOrderText(order)}`).join("\n\n----------------\n\n");
        try {
            await bot.api.sendMessage(host.telegramId, message + ordersText);
            sent++;
        }
        catch (e) {
